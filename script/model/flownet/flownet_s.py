...
def get_flownet(dim_img, max_step, trainable=True):
    with tf.variable_scope('FlowNetS'):
        input_a = tf.placeholder(tf.float32, shape=[max_step] + dim_img, name='input_a') #b,l,h,d,c
        input_b = tf.placeholder(tf.float32, shape=[max_step] + dim_img, name='input_b') #b,l,h,d,c
        concat_inputs = tf.concat([input_a, input_b], axis=3)
        _, height, width, _ = input_a.shape.as_list()
        with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
                            # Only backprop this network if trainable
                            trainable=trainable,
                            # He (aka MSRA) weight initialization
